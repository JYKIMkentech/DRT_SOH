...
for s = 1:num_scenarios
    ...
    noise_std_1per = 0.01 * mean(V_est);  % 1% of the maximum voltage
    ...
    noise_std_2per = 0.02 * mean(V_est);  % 2% of the maximum voltage
...
for s = 1:num_scenarios
    ...
    % Add 1% Gaussian noise
    noise_std_1per = 0.01 * mean(V_est);  
    V_sd_1per = V_est + noise_std_1per * randn(size(V_est));
...
save_dir = 'G:\공유 드라이브\Battery Software Lab\Projects\DRT\SD';
...
% Save the structs to the specified directory
save(fullfile(save_dir, 'AS1_1per.mat'), 'AS1_1per');
save(fullfile(save_dir, 'AS1_2per.mat'), 'AS1_2per');
save(fullfile(save_dir, 'AS2_1per.mat'), 'AS2_1per');
save(fullfile(save_dir, 'AS2_2per.mat'), 'AS2_2per');
