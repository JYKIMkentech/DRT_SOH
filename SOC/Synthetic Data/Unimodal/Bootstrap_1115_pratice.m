...
num_scenarios = length(type_data);  % 시나리오 수

% 시나리오 번호 리스트 추출
SN_list = [type_data.SN];

% 사용자에게 선택된 데이터셋, 타입, 시나리오 번호 출력
...
for s = 1:num_scenarios
    ...
    V_sd = scenario_data.V;        % 측정 전압 (열 벡터)
    ik = scenario_data.I;          % 전류 (열 벡터)
    t = scenario_data.t;           % 시간 벡터
    dt = scenario_data.dt;         % dt 값
    duration = scenario_data.dur;  % duration 값
    n = scenario_data.n;           % n 값 (RC 요소의 개수)
    lambda = 0.518; %scenario_data.Lambda; % 람다 값 (필요 시 수정)
